...
public class SceneFragment extends Basecfragment implements
    ...
    protected void onView() {
        ...
        adapter = new ScenefargmentAdapter(getActivity(), list, listint, listintwo, false);
        ...
        refresh_view.setXRefreshViewListener(new XRefreshView.SimpleXRefreshListener() {
            ...
            public void onRefresh(boolean isPullDown) {
                ...
                if (boxnumberre.equals("")) {
                    ...
                    listintwo.clear();
                    list.clear();
                    adapter = new ScenefargmentAdapter(getActivity(), list, listint, listintwo, true);
                    gritview_id.setAdapter(adapter);
    ...
    private void get_all_scene(final Map<String, Object> map) {
        MyOkHttp.postMapObject(ApiHelper.sraum_getAllScene2, map, new Mycallback(new AddTogglenInterfacer() {
            @Override
            ...
            public void onSuccess(User user) {
                ...
                if (manuaList != null) {
                    for (User.manua ud : manuaList) {
                        listtype.add(ud.sceneStatus);
                    }
                    for (User.manua us : manuaList) {
                        Map map = new HashMap();
                        map.put("type", us.type);
                        map.put("name", us.name);
                        map.put("sceneStatus", us.sceneStatus);
...
//                gritview_id.setAdapter(adapter);
                adapter.setList_s(list, listint, listintwo, true);
...
//        }
    ...
    public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
        scename = list.get(position).get("name").toString();
        type = list.get(position).get("type").toString();
...
//            map.put("type", type);
    ...
    private void sraum_sceneControl(final int position, final Map<String, Object> map) {
        ...
        switch (type) {
            case "101":
                method = ApiHelper.sraum_manualControl;
                break;
            default:
                method = ApiHelper.sraum_sceneControl;
                break;
        }
