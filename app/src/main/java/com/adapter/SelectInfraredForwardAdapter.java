...
//    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (null == convertView) {
            ...
            convertView = LayoutInflater.from(activity).inflate(R.layout.select_infrared_forward_item, null);
        ...
        viewHolderContentType.gateway_name_txt.setText((String) list.get(position).get("controllerId"));
        ...
        viewHolderContentType.swipe_content_linear.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (refreshListener != null)
                    refreshListener.onItemCLick(position);
            }
        });
        return convertView;
    }
...
//        dialog.setCanceledOnTouchOutside(false);// 设置点击屏幕Dialog不消失
        ...
        confirm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                sraum_deleteWifiApple(linkId, dialog);
            }
        });
    }
...
//        mapdevice.put("boxNumber", TokenUtil.getBoxnumber(LinkageListActivity.this));
        MyOkHttp.postMapString(ApiHelper.sraum_updateWifiAppleName, mapdevice, new Mycallback(new AddTogglenInterfacer() {
            @Override
...
//                refreshLayout.autoRefresh();
     ...
     * @param dialog
     */
    private void sraum_deleteWifiApple(final String number, final Dialog dialog) {
        Map<String, String> mapdevice = new HashMap<>();
        mapdevice.put("token", TokenUtil.getToken(activity));
        mapdevice.put("number", number);
//        mapdevice.put("boxNumber", TokenUtil.getBoxnumber(LinkageListActivity.this));
        MyOkHttp.postMapString(ApiHelper.sraum_deleteWifiApple, mapdevice, new Mycallback(new AddTogglenInterfacer() {
            @Override
            public void addTogglenInterfacer() {//刷新togglen数据
                sraum_deleteWifiApple(number,dialog);
            }
        }, activity, dialogUtil) {
            @Override
            public void onError(Call call, Exception e, int id) {
