...
public class DownloadThread extends Thread {
    ...
    public DownloadThread(DownloadProgressListener listener, FileDownloadered downloader, URL downUrl, File saveFile, int block, int downLength, int threadId) {
    ...
    public void run() {
        if (downLength < block) {//未下载完成
            try {
                ...
                byte[] buffer = new byte[1000 * 1024];           //设置本地数据的缓存大小为1MB
                ...
                while (!downloader.getExited() && (offset = inStream.read(buffer, 0, 1000 * 1024)) != -1) {
                    //8192 = offset,服务器端推流每次8kb.
                    ...
                    Log.e("robin debug","offset:" + offset + ",downLength:" +downLength);
                }
                threadfile.close();
                inStream.close();
