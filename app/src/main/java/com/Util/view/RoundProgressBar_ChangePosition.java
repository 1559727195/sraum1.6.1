 ...
 * @author xiaanming
 */
public class RoundProgressBar_ChangePosition extends View {
    /**
     * 内圆的半径
     */
    ...
    public RoundProgressBar_ChangePosition(Context context) {
        this(context, null);
    }

    public RoundProgressBar_ChangePosition(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RoundProgressBar_ChangePosition(Context context, AttributeSet attrs, int defStyle) {
...
//        canvas.translate(-getHeight(),0);
        ...
        int radius_innner = (int) (centre - roundWidth_inner / 2); //圆环的半径
        paint.setColor(roundColor); //设置圆环的颜色
        paint.setStyle(Paint.Style.STROKE); //设置空心
        paint.setStrokeWidth(roundWidth); //设置圆环的宽度
        paint.setAntiAlias(true);  //消除锯齿
        canvas.drawCircle(centre, centre, radius, paint); //画出圆环
        canvas.drawCircle(centre, centre, radius_innner, paint); //画出圆环
        Log.e("log", centre + "");
...
//        float textWidth = paint.measureText(percent + "%");   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间
        ...
        //设置进度是实心还是空心
//        paint.setStrokeWidth(radius - radius_innner); //设置圆环的宽度
        paint.setStrokeWidth(roundWidth); //设置圆环的宽度
        paint.setColor(roundProgressColor);  //设置进度的颜色
        RectF oval = new RectF(centre - radius_innner - (radius - radius_innner) / 2, centre - radius_innner - (radius - radius_innner) / 2, centre
                + radius_innner + (radius - radius_innner) / 2, centre + radius_innner + (radius - radius_innner) / 2);  //用于定义的圆弧的形状和大小的界限
